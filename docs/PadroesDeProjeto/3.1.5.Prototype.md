...
```
class Art implements ArtPrototype {
    ...
    public showDetails(): void {
        ...
        console.log(`Tamanho da Imagem (Base64): ${this.image ? this.image.length : 0} caracteres`);
...
class ArtService {
    ...
    private prototypeRegistry: Map<string, Art> = new Map();
    ...
    public getArtById(id: number): Art | undefined {
...
class Client {
    ...
    public run(): void {
        ...
        if (storedArt1 && storedArt2) {
            ...
            console.log(`Os objetos art1 e art2 são instâncias diferentes? ${storedArt1 !== storedArt2}`);
            console.log(`O autor de art1 é o mesmo do protótipo? ${storedArt1.author === storedArt2.author}`);
        }
    }
}
...
const appClient = new Client(artService);
appClient.run();
```

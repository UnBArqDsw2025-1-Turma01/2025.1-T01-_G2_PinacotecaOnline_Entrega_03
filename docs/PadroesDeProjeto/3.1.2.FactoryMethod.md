...
O Factory Method (Método de Fábrica) é um padrão criacional de projeto que fornece uma interface para criar objetos em uma superclasse, mas permite que as subclasses alterem o tipo de objetos que serão criados[2]. Esse padrão é amplamente utilizado para promover a flexibilidade na criação de objetos, delegando a responsabilidade de instanciação às subclasses. É particularmente útil em frameworks e sistemas em que o tipo exato de objeto a ser criado não pode ser determinado antecipadamente pela classe base.
...
> [2] Refactoring Guru. (s.d.). *Factory Method*. Disponível em: <https://refactoring.guru/design-patterns/factory-method>.
...
| 1.4 | 31/05/2025 | Linkando os códigos para nova branch e mudando o passo a passo da instalação     |  [Renan Araújo](https://github.com/renantfm4), [Danielle Rodrigues](https://github.com/Danizelle)                                 | -                                             |
